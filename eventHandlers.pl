...
handleLeftClick([OLD,MINESMAP,STATEMAP,P]) :- get(@w, value, A), atom_number(A, 0), getTilePos(OLD,X,Y), gridToMap(X1,Y1,X,Y), \+revealed(X1,Y1), getTile(X1,Y1,STATEMAP,State), State is 100, getTile(X1,Y1,MINESMAP,Val), Val is -1, setTile(STATEMAP,X1,Y1,111,NEWSTATEMAP), assertz(exploded(X1,Y1)), !, swapIcon([OLD,'./icons/hit.xpm',MINESMAP,NEWSTATEMAP,P]), stopCounter, uncoverMap(P,MINESMAP).
